...
### 2026-07-04 — One-command dev + PWA (Claude Code)
- ROOT `package.json`: `npm run dev` (concurrently, `[backend]`/`[frontend]` prefixed, `-k` kills
  both together), plus install:all / check / test / migrate. README.md added at root.
- PWA via `vite-plugin-pwa` (frontend): manifest (standalone, portrait, theme #1a2434 /
  bg #f7faff), icons generated by `frontend/scripts/generate-icons.mjs` (sharp renders an inline
  SVG "M"-path mark — no font dependency; run `npm --prefix frontend run icons` to regenerate;
  outputs pwa-192/512, maskable-512 w/ safe zone, apple-touch-icon-180, favicon.svg into public/).
- Service worker decisions: `registerType: "prompt"` + `ui/ReloadPrompt.vue` toast (never
  auto-reload mid-exercise); SPA `navigateFallbackDenylist: [/^\/api\//]`; runtime caching ONLY
  for Google Fonts (SWR stylesheets, CacheFirst webfonts) — `/api` is never cached;
  `devOptions.enabled: false` (no SW in dev — stale-cache misery). Types via
  `vite-plugin-pwa/vue` reference in vite-env.d.ts; registration happens through the
  `virtual:pwa-register/vue` import in ReloadPrompt (no head script).
- iOS: apple-touch-icon + mobile-web-app-capable/status-bar meta in index.html.
- Verified: build emits sw.js + manifest (15 precache entries); compiled SW contains the
  fallback denylist + font rules; vite preview serves all assets with correct MIME types.
- REMEMBER: installing on a phone requires HTTPS deployment (user is handling deploy). TWA/native
  deliberately rejected — PWA fits a personal cross-platform app.
