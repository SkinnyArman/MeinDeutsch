...
test("goal is incomplete when any section has no activity", () => {
  const steps = computeDailyGoalSteps({
    countsToday: { dailyTalk: 1, alltagssprache: 2, kollokationen: 0, vocabulary: 1 },
    vocabularyDueNow: 3
  });
...
test("goal completes when every section has at least one task", () => {
  const steps = computeDailyGoalSteps({
    countsToday: { dailyTalk: 1, alltagssprache: 1, kollokationen: 1, vocabulary: 1 },
    vocabularyDueNow: 5
  });
...
test("vocabulary counts as done when nothing is due, but not otherwise", () => {
  const emptyQueue = computeDailyGoalSteps({
    countsToday: { dailyTalk: 1, alltagssprache: 1, kollokationen: 1, vocabulary: 0 },
    vocabularyDueNow: 0
  });
