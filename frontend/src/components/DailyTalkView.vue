...
const highlightedAnswerSegments = computed(() => {
  ...
  const rangesFromErrors = buildHighlightedSegments(result.value.answerText, result.value.errorTypes);
  const hasHighlights = rangesFromErrors.some((segment) => segment.highlight);
  if (hasHighlights) {
    return rangesFromErrors;
  }
